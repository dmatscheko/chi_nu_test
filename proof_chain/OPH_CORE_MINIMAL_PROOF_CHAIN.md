...
6. *(stretch, mathematics — sharpened by T18 + T20, halved by T25)* the
   ...
   arbitrary-subset question now has machine-checked walls; **and the
   residual question itself is FACTORED in v10** (`Rule90Parity.lean`,
   `Rule90TwoPower.lean`, `Rule90Diagonal.lean`, §14): on even rings
   failure ⟺ a single-parity ghost (T38 — arbitrary subsets classify
   sector-by-sector; the residue is Rule-60 zero-set rigidity on the half
   ring), on two-power rings the worldline case is closed outright (T39),
   and the lone lightlike diagonal is a NEW counting-tight family
   (T40/T41); what remains is the odd-ring order structure and the
   rigidity statements), and other
   linear CA
...
## 14. The v10 campaign (2026-07-10, seventh pass): the simulation's findings become the theorems

**The pipeline, run twice in one day.** The `oph_sim` v2 rebuild
(2026-07-10, same day) produced finding R1 and conjectures C1–C5
(`oph_sim/FINDINGS.md` Part IV). This pass formalized the top of that
list — and the probes written to prepare the formalization surfaced two
theorems nobody had conjectured, which were then formalized too
(FINDINGS.md Part V is the simulation-side account).

**The verification half.** Every lemma was probe-verified numerically
BEFORE its Lean transcription (the bridge identity for all `n = 3..20`;
the doubling lemma; sharp nilpotency; the all-ones funnel on 3000 seeds;
the T38 statement itself on 4096 exhaustive + randomized fresh subsets;
T39 exhaustively at `(8,3)` and sampled at `(16,7)`, `(16,11)`,
`(32,15)`, `(64,31)` — where the first `n = 32` sweep "failed" 1200/1200
because of a JavaScript `1 << 32` int wrap, caught within minutes by the
probe's own theorem anchors, the same failure mode the v2 build's battery
had already taught; the BigInt re-run decodes 100%). Fresh `lake build`:
**8287 jobs, clean**. Two-namespace environment sweep: **1284**
non-internal theorem/def declarations, **0 `sorryAx`, 0 non-standard
...
Consequences, all unconditional in `n, t, S`: an arbitrary cell set is an
information set iff no nonzero single-parity ghost is dark on it
(`isInformationSet_iff_single_parity_ghost`), i.e. failure ⟺ a
single-parity shadow (`not_isInformationSet_iff_single_parity_shadow`) —
the containment half of the simulation's shadow-atlas conjecture C1. And
the finding R1 itself: in half-ring coordinates (`u ↦ i + b + 2u`,
`halfEmbed`) each sector evolves by **Rule 60 on `ZMod (n/2)`**
(`sectorTrace_succ`, `sectorTrace_eq_iterate`). The module also proves
the **bridge** every other v10 module runs on:
...
killed by one step is constant; so the last nonzero iterate of any
nonzero ghost is the **all-ones row**, at depth `s ≤ n−1 ≤ 2t+1`. Row
`i = ⌊s/2⌋` of ANY pair screen sees it: at even `s` one cell reads `1`;
at odd `s` the adjacent pair reads two zeros summing to `1`. Hence
`pairScreen_isInformationSet_iff_two_pow` /
`pathScreen_isInformationSet_iff_two_pow`: EVERY worldline — teleports,
superluminal sprints, no causality hypothesis — decodes iff
`n ≤ 2(t+1)`. T36's `(8,3)` all-decode exhibit is the `k = 3` instance;
...
kill (the triangular recursion
`T^[i]w(u+1) = T^[i]w(u) + T^[i+1]w(u)` zips `w` to zero on `t+1`
consecutive sites), the lone diagonal — ONE cell per row — is an
information set on odd rings **iff `n ≤ t+1`**
(`diagScreen_isInformationSet_iff_odd`): at `t+1 = n` it meets the
universal counting bound EXACTLY, the first counting-tight screen family
in the tree — while the timelike single column never decodes at any
horizon (v3), so the lightlike boost flips the one-cell observer from
hopeless to optimal. On even rings the lone diagonal reads one T38
sector and never decodes; two diagonals decode iff their bases have
opposite parity, at the sharp `n ≤ 2(t+1)`, at ANY offset
(`diagScreen_pair_isInformationSet_iff_even` — T18a made offset-free);
same-parity pairs never. Kernel instances: `diag_five_four` (five cells,
five dimensions, zero slack), `diagPair_six_two`,
`pairScreen_teleport_4_1`.

**What the probes enshrined beyond the theorems** (recorded,
machine-testable, open): Rule-60 zero-set **rigidity** — C1's exactness
half — holds through `m = 13` with minimal rigid window exactly
`⌊m/2⌋` (sharper than conjectured: half of capacity suffices); the
**left-crawl law** (single-cell Rule-60 readers with steps in `{0,−1}`
always decode — exhaustive `m = 3, 5, 7`; the R1b-image of T36, yet true
on odd rings where R1b does not apply); the odd-ring **diagonal-pair
window** (two diagonals decode ⟺ `n − t − 1 ≤ 2⁻¹(j₁−j₀) ≤ t + 1`,
probe-exact `n = 7..13`; sufficiency is the prefix-kill covering
argument on paper, necessity open); and the teleport ghost plateau
(rank exactly `(m+1)/2` at every horizon — C4, now reduced via R1b to
one family of single-cell Rule-60 readers).

**What the v10 pass did NOT change.** The physics ledger is untouched —
SEE, MAR, L0–L7, G9 (channel identification + numeric S), the G10 ledger
+ convention, P's source branch, BW + scaling limit, Γ; the experiment's
epistemic position is exactly §8.3's, and nothing hardware-touching
changed. Open mathematics after v10: the odd-ring order structure (C2)
and the rigidity statements (R2 = C1 exactness), plus the named probe
laws above — the even-ring and two-power-ring halves of the old item are
closed, and every remaining item has a decidable per-instance test and
