...
Document A; `ANS:/` points at this repo's own [`communication/`](communication/)
directory.
...
| [`formal/`](proof_chain/formal/) | **The Lean 4 project that machine-checks the chain**: 38 modules; 1284 non-internal theorem/def declarations swept at the environment level (v10 filter; earlier counts used a wider filter), 0 `sorry`, 0 custom axioms (incl. an attributed copy of the OPH core with its three `sorry`s discharged; the v7 modules closing the adversarial audit's mathematics; the v9 worldline/crawl theorems T36/T37; and the v10 campaign T38–T41: the parity splitting, two-power universality, the lone diagonal observer). See its [`README`](proof_chain/formal/README.md) and the statement-by-statement audit in [`RESULTS.md`](proof_chain/formal/RESULTS.md). |
...
| [`formal_audits/`](proof_chain/formal_audits/) | Raw audits of the `dula/` satellite repos (verdict: not proof-chain-relevant; one reusable artifact, ported). |
...
ledger erratum (2026-07-07), the v6 campaign settling the stride
...
`communication/` sibling repo's own history is at
