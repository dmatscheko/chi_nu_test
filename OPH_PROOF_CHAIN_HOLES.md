...
| v9 response (added post-audit; the audit text is frozen) | The maintainers' sixth pass (proof-chain **v9**, 2026-07-09) closes this audit's **last remaining open mathematics row**: F6 → **T36** (`Rule90Lipschitz.lean` — the Lipschitz worldline theorem). The conjecture this audit graded "two data points" is now a theorem *stronger than the chain ever conjectured*: the sharp threshold `n ≤ 2(t+1)` is invariant not merely across rational slopes `0 ≤ p/q ≤ 1` but across **every 1-Lipschitz worldline** (zigzags and reversals included), with *complete local decodability* (`pathScreen_closure_complete` — the propagation closure is the whole block, so decoding needs no global linear algebra), sharp uniformly in the path (`pathScreen_isInformationSet_iff`), and the conjectured statement itself as the corollary `slopeTube_isInformationSet_iff` (every `p ≤ q`, every `n, t`, every base point; T9 and T18a become the two extreme corollaries; the v8 sample instances re-derive). The engine is a two-chain fan induction in which the 1-Lipschitz telescope is exactly the condition keeping each level's screen pair inside the previous level's inferred interval — the v8-recorded sheared-CA attack was not needed. The pass also delimits what remains: beyond the Lipschitz class the pair-screen landscape is **provably wild** — kernel-checked: at `(6,2)` the complete classification is order-sensitive ("last step Lipschitz", `pairScreen_class_6_2`; `![0,0,2]` fails while `![0,2,2]` decodes), at `(8,3)` ALL `8^4` paths decode, at `(10,4)` the slope-2 line and the late jump fail at exact capacity (sweep artifact: `formal/evidence/path_screen_sweep.txt`). The pass also closed T30's own named leftover: **T37** (`Rule90Crawl.lean`) — at the sharp threshold the gap-2 screen's propagation closure is complete **iff the ring is odd** (the simulation's crawl made into a proof; the odd `g = 2` decode re-derives through propagation), so two-column local decodability is classified at every ring distance. Mathematics-side residue after v9: the arbitrary-subset classification alone, now with machine-checked walls. Environment sweep after v9: 35 modules, 1235 non-internal theorem/def declarations in both namespaces, 0 sorry, standard axioms only. Campaign log: chain §13 |
| v10 response (added post-audit; the audit text is frozen) | The maintainers' seventh pass (proof-chain **v10**, 2026-07-10) **factors the residue this audit left standing**. The v9 residue — "the arbitrary-subset classification alone" — splits by theorem: **T38** (`Rule90Parity.lean`) proves that on **even** rings an arbitrary cell set fails ⟺ a nonzero **single-parity** ghost is dark on it (unconditional in `n, t, S` — the even-ring half of the item reduces to Rule-60 zero-set rigidity on the half ring, numerically rigid through `m = 13` with minimal rigid window exactly `⌊m/2⌋`), because the spacetime block is two non-interacting parity sectors, each literally Rule 60 on `ZMod (n/2)` (`traj_parityProj`, `sectorTrace_succ`); **T39** (`Rule90TwoPower.lean`) closes the worldline case outright on **two-power** rings — every pair screen decodes iff `n ≤ 2(t+1)`, teleports included, no causality hypothesis (the `(8,3)` all-decode wall this audit's F6 discussion inherited is the `k = 3` instance; the proof is elementary: a doubling lemma makes Rule 60 nilpotent and the last nonzero iterate of any ghost is the all-ones row, which an adjacent pair cannot miss); and **T40/T41** (`Rule90Diagonal.lean`) add a screen family this audit could not have demanded because nobody had conjectured it: the lone lightlike diagonal — ONE cell per row — decodes odd rings **iff `n ≤ t+1`**, meeting the universal counting bound exactly (the first counting-tight family; the timelike single column provably never decodes), never decodes even rings, and pairs of opposite-parity diagonals are sharp at any offset. All four originated as `oph_sim` findings/probes, each probe-verified before transcription (including a `1 << 32` int-wrap false negative caught by the probes' own theorem anchors — the discipline this audit's F22 asked for, now standard). Mathematics-side residue after v10: the odd-ring order structure (C2) and the rigidity statements (R2), plus named probe-exact laws (left-crawl; the odd diagonal-pair window), every one with a decidable per-instance test and an evidence artifact. Environment sweep after v10: 38 modules, 1284 non-internal theorem/def declarations in both namespaces, 0 sorry, standard axioms only. Campaign log: chain §14 |
