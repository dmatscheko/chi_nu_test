...
export default {
  ...
  async mount({ stage, legend }) {
    ...
    stage.append(
      ...
      el('div', { class: 'card' }, el('h3', { html: 'E7/E9 — the reduction and the conjectures handed to the next formal campaign' }),
        table(['#', 'statement', 'evidence', 'proof idea'],
          [[td('<b>R1</b>'), td('<b>the parity/Rule-60 splitting</b>: on even n, the Rule-90 block IS two independent Rule-60 systems on ℤ/(n/2) (sector s = (j−i) mod 2, u = (j−i−s)/2; y′ = y + shift y); cells, kernels AND the propagation closure all split'), td('23,600 cells exact; kernel(S) = ker₀⊕ker₁ on 300 random subsets, exact', 'good'), td('every constraint is intra-sector; one-line induction per direction. Biggest downstream yield: proves C1\'s containment outright', 'dimc')],
           [td('C1'), td('even n ≤ 2(t+1): the maximal ghost shadows are exactly the 2^{n/2+1}−2 single-parity-class ghosts'), td(laws && laws.C1holds ? 'exhaustive n ≤ 14, t up to 2n ✓' : '—', 'good'), td('= R1 (containment, done) + R2: Rule-60 zero-set rigidity on the half-ring', 'dimc')],
           ...
           [td('C3'), td('n = 2ᵏ at capacity: EVERY worldline pair-screen decodes'), td('exhaustive n=8; 0 failures in 3.2M samples at n=16/32/64', 'good'), td('u-adic valuation ladder in the local ring (sketch above); via R1: two nilpotent Rule-60 sectors', 'dimc')],
           [td('C4'), td('teleport-n/2 worldline: permanent ghost ⟺ n/2 odd; kernel dim n/2 − 1, split (n/2−1)/2 per sector'), td('n ≤ 26, t ≤ 24; sector split verified at n=10, 14; antiphase period-2 mechanism proven at n=6 (special)', 'good'), td('via R1: a slope-2 single-cell-per-row reader of Rule 60 on the odd half-ring', 'dimc')],
        ...
        el('div', { class: 'hint', html: 'The pattern of this project: FINDINGS items 1/3/10/11 of the first build became theorems T30/T31/T36/T37 of the chain. R1 + C1–C5 are the next batch, each machine-testable (decidable per instance), each with its evidence artifact in data/experiments.json; R1 and C3 carry proof sketches (FINDINGS.md items 18 and 24).' })),
    );
